"""Native private R5 play maps; ordinary input only, never a capture director.

Run CornerPlayableR5 once, then CornerPlayableR5Reload in a NEW commandlet.
The latter is read-only and publishes the launch manifest. No UE/UI launch here.
Source R5, Hero, eight IdleR2 BPs and the native vehicle are never saved/edited.
"""
...
OWNER='HarborCity_M5VS2_CornerPlayableR5'; SCHEMA='HarborCity.M5VS2.CornerPlayableR5.v1'
...
assert PHASE in ('CornerPlayableR5','CornerPlayableR5Reload') and OUT.is_relative_to(DOC/'editor_runtime')
assert not (OUT/'author_result.json').exists()
OUT.mkdir(parents=True,exist_ok=True)
TOKEN=hashlib.sha256(str(OUT).encode()).hexdigest()[:12]
ROOT='/Game/HarborCity/M5VS2/WorldRev2/PlayableR5_'+TOKEN
R=dict(schema=SCHEMA,art_revision=5,status='RUNNING',phase=PHASE,owner=OWNER,checks=[],assets=[],inputs={},maps={},
       ...
       scope='50 m R5 corner, walking/flight/driving/proportion preview. NPC idle/face only; no M3Experience, combat/dialogue/quests not enabled or accepted.')
PROTECTED={}; SOURCES={}
...
def resolve_base_hero():
    ...
    R['inputs']['hero_selection']=binding(file,'evidence');R['base_selected_hero']=bp
    return cls(bp)


def hero_readers():
    # Isolated function namespace: Hero transform serialization must not replace
    # the NPC/environment transform reader already installed in this author.
    reader=WORK/'tools/ue_m5_vs2_hero_rev2_integration_author.py';protect(reader,kind='script')
    text=reader.read_text('utf-8-sig');wanted=('vec','quat','transform','value','props','cdo_objects','preserved','scs_components')
    nodes=[n for n in ast.parse(text).body if isinstance(n,ast.FunctionDef) and n.name in wanted]
    check('exact eight existing Hero read-only function names',tuple(n.name for n in nodes)==wanted)
    code='\n\n'.join(ast.get_source_segment(text,n) for n in nodes)
    ns=dict(U=U,check=check,path=path,package=package)
    exec(compile(ast.Module(body=nodes,type_ignores=[]),str(reader)+'::readonly_hero','exec'),ns)
    modest=WORK/'tools/ue_m5_vs2_hero_modesty_author.py';protect(modest,kind='script');body=modest.read_text('utf-8-sig')
    block=[n for n in ast.parse(body).body if isinstance(n,ast.FunctionDef) and n.name=='baseline']
    check('one original safety-layer baseline reader',len(block)==1)
    exec(compile(ast.Module(body=block,type_ignores=[]),str(modest)+'::readonly_baseline','exec'),ns)
    R['hero_native_readers']=dict(integration_path=str(reader),integration_functions_sha256=hashlib.sha256(code.encode()).hexdigest(),
        modesty_path=str(modest),baseline_sha256=hashlib.sha256(ast.get_source_segment(body,block[0]).encode()).hexdigest())
    return ns

def resolve_hero():
    resolve_base_hero()
    f,data=latest('author_*_HeroModestyReload',lambda d:d.get('schema')=='HarborCity.M5VS2.HeroModesty.v1'
                  and d.get('phase')=='HeroModestyReload' and d.get('fresh_process_disk_readback')=='PASS')
    check('actual safety-layer fresh Reload preserves all old assets',data.get('owner')=='HarborCity_M5VS2_HeroModesty'
          and data.get('old_asset_writes')==0 and data.get('map_writes')==0 and data.get('selection_file_written') is False
          and data.get('preservation',{}).get('changed_source_files')==[] and len(data.get('assets',[]))==3)
    ref=data['inputs']['apply'];prior=report(ref['path'],ref['sha256'])
    check('safety-layer Reload exactly matches actual Apply',prior.get('phase')=='HeroModestyApply'
          and prior.get('schema')==data['schema'] and all(prior.get(k)==data.get(k) for k in
          ('blueprint','source_blueprint','source_body','source_baseline','native_readback','assets')))
    history=data['preservation']['source_sha256_before'];old_module=PROJECT/'Binaries/Win64/UnrealEditor-HarborCity.dll'
    historical_modules=[dict(path=p,sha256=h,scope='HISTORICAL_AUTHOR_DLL_NOT_REQUIRED_TO_EQUAL_CURRENT_DIAGNOSTIC_BUILD')
                        for p,h in history.items() if Path(p).resolve()==old_module.resolve()]
    check('one historical author DLL proof retained',len(historical_modules)==1)
    required_helpers={str((PROJECT/'Source/HarborCity/M5VS2'/('HCM5VS2HeroModestyComponent'+ext)).resolve()) for ext in ('.h','.cpp')}
    matched_helpers=set()
    for filename,expected in history.items():
        saved=Path(filename).resolve()
        if saved==old_module.resolve():continue  # sole authorized historic-binary exception
        kind=next((kind for kind,root in (('package',PROJECT/'Content'),('config',PROJECT/'Config'),
            ('cpp',PROJECT/'Source'),('script',WORK/'tools'),('evidence',DOC)) if saved.is_relative_to(root)),None)
        check('known original safety-layer provenance root',kind is not None,str(saved))
        protect(saved,expected,kind)
        if str(saved) in required_helpers:matched_helpers.add(str(saved))
    check('original safety-layer helper h/cpp byte identity',matched_helpers==required_helpers)
    check('safety layer derives from unchanged explicit Hero selection',data['source_blueprint']==R['base_selected_hero'])
    select_ref=prior['inputs']['selection'];check('same original selection bytes',select_ref['sha256']==R['inputs']['hero_selection']['sha256'])
    assets(data['assets']);bp=package(data['blueprint'])
    check('exact private safety-layer class namespace',re.fullmatch(r'/Game/HarborCity/M5VS2/HeroModesty/Review_[0-9a-f]{12}/BP_HeroSafetyShorts',bp) is not None)
    ns=hero_readers();base=ns['baseline'](load(R['base_selected_hero']));actual=ns['baseline'](load(bp))
    check('source and candidate retain complete actual baseline',base==data['source_baseline'] and actual==base)
    check('five original body material slots and first-person arms retained',len(actual['materials'])==5
          and actual['materials']==base['materials'] and actual['first_person_arms']==base['first_person_arms']
          and actual['first_person_arms'] is not None)
    for path_value in [actual['mesh'],actual['animation'],actual['first_person_arms'],*actual['materials']]:
        protect(disk(path_value),package=package(path_value))
    components=[c for c in ns['scs_components'](load(bp)) if isinstance(c,U.HCM5VS2HeroModestyComponent)]
    check('exactly one actual safety-layer native component',len(components)==1)
    component=ns['props'](components[0],('safety_shorts','cloth_material'))
    check('same actual garment mesh and material as Reload',component==data['native_readback']['component'])
    topology=json.loads(U.HCM5VS2HeroModestyEditor.inspect_safety_shorts(load(data['source_body']),load(data['shorts'])))
    check('actual closed garment native readback matches Reload',topology.get('status')=='PASS' and topology==data['native_readback']['topology'])
    R['inputs']['hero_modesty_reload']=binding(f,'evidence');R['inputs']['hero_modesty_apply']=binding(ref['path'],'evidence')
    R['selected_hero']=bp;R['hero_safety']=dict(status='NATIVE_RELOAD_SELECTED_NOT_ALL_ANGLE_ART_APPROVED',
        blueprint=bp,source_blueprint=R['base_selected_hero'],selection=R['inputs']['hero_selection'],
        apply=R['inputs']['hero_modesty_apply'],reload=R['inputs']['hero_modesty_reload'],
        baseline=actual,component=component,topology=topology,historical_author_modules=historical_modules,
        current_module_binding='SEPARATE_CURRENT_MANIFEST_MODULE_SHA',visual_coverage='USER_REVIEW')
    return cls(bp)
...
def spawn(label,kind,position=(0,0,0),yaw=0):
    ...
    actor.set_actor_label('HC_VS2_PlayR5_'+label);actor.set_editor_property('tags',[U.Name(OWNER)])
    return actor
...
def snapshot_map(period,hero_class,specs,expected_flight,expected_vehicle):
    ...
    check('native stable car identity',str(vehicles[0].get_editor_property('stable_id'))=='VS2_PlayR5_Car')
    ...
    for letter,spec in specs.items():
        ...
        check('NPC stationarity and private stable identity',row['stationary'] and row['stable_id']=='VS2_PlayR5_NPC_'+letter)
...
def resolve_source():
    file,source=latest('author_*_CornerVTwo',lambda d:d.get('owner')=='HarborCity_M5VS2_HarborCorner_Rev2' and d.get('art_revision')==5)
    # R5's actual layout additionally proves the bounded authored scope.
    ...
    check('exact saved R5 layout and original boundary',layout.get('schema_version')==2 and layout.get('art_revision')==5 and layout.get('playable_bounds_xy')==[-2500,-2500,2500,2500],layout.get('playable_bounds_xy'))
    ...
    R['source_corner_proof']=dict(art_revision=5,author=R['inputs']['source_corner'],layout=R['layout'],map=source['map'])
    windmill=next(h for h in source['houses'] if h['role']=='WindmillLandmark')
    box=windmill['bounds_cm'];R['windmill']=dict(source_role=windmill['role'],bounds_cm=box,center_xy_cm=[(box['min'][i]+box['max'][i])/2 for i in (0,1)],maximum_world_z_cm=box['max'][2],scope='Actual R5 native component bounds, static obstacle evidence; not a collision-free flight route.')
...
def populate(period,map_path,source,layout,hero_class,specs,mode,expected_vehicle):
    ...
    check('native independent R5 map duplicate',A.duplicate_asset(source['map'],map_path) is not None)
    ...
    for letter,(x,y,z,yaw) in posts.items():
        ...
        actor.set_editor_property('stable_id',U.Name('VS2_PlayR5_NPC_'+letter))
    car=spawn('ParkedVehicle',U.HCM1Vehicle,(70,-950,105),90);car.set_editor_property('stable_id',U.Name('VS2_PlayR5_Car'))
    ...
    check('save only independent R5 play map',LEVEL.save_current_level());check('same-process reload private map',LEVEL.load_level(map_path))
...
try:
    ...
    if PHASE=='CornerPlayableR5':
        ...
        check('private namespace absent',not A.does_directory_exist(ROOT));mode_path=ROOT+'/BP_PlayableR5GameMode'
        ...
        for period in PERIODS:populate(period,ROOT+'/L_CornerPlayableR5_'+period,source,layout,hero_class,specs,mode,car_baseline)
        R['source_bindings']=list(SOURCES.values());R['status']='PASS'
        R['pass_scope']='Native creation and same-process persistence only; separate CornerPlayableR5Reload required before launch.'
    else:
        file,prior=latest('author_*_CornerPlayableR5',lambda d:d.get('schema')==SCHEMA and d.get('owner')==OWNER)
        check('completed private source author exactly three maps',set(prior['maps'])==set(PERIODS) and len(prior['assets'])==4)
        ROOT=prior['destination'];check('precise independent destination',re.fullmatch(r'/Game/HarborCity/M5VS2/WorldRev2/PlayableR5_[0-9a-f]{12}',ROOT) is not None)
        for row in prior['source_bindings']:protect(row['path'],row['sha256'],row['kind'],**({'package':row['package']} if 'package' in row else {}))
        assets(prior['assets']);hero_class=resolve_hero();specs=resolve_npcs()
        check('same selected safety-layer candidate as Apply',R['selected_hero']==prior['selected_hero'] and R['hero_safety']==prior['hero_safety'])
        R['source_corner_proof']=prior['source_corner_proof']
        check('strict R5 source proof preserved',prior.get('art_revision')==5 and R['source_corner_proof'].get('art_revision')==5)
        ...
        for period,row in prior['maps'].items():
            check('exact period map path',row['package']==ROOT+'/L_CornerPlayableR5_'+period)
        ...
        manifest=dict(schema=SCHEMA,art_revision=5,source_corner_proof=R['source_corner_proof'],hero_safety=R['hero_safety'],status='READY_FOR_PLAY_NOT_RUNTIME_TESTED',owner=OWNER,kind=R['kind'],destination=ROOT,
            ...
            save_slot_prefix='HarborCity_VS2_PlayR5_'+ROOT.rsplit('_',1)[-1]+'_',
            ...
            scope=R['scope'])
        target=OUT/'corner_playable_r5_manifest.json';target.write_text(json.dumps(manifest,ensure_ascii=False,indent=2)+'\n','utf-8')
...
if R['status']!='PASS':raise RuntimeError('Private playable R5 author failed; retain all failure evidence and unique partial assets.')

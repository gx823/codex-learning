...
[CmdletBinding()]
param([Parameter(Mandatory=$true)][string]$AuthorReport,[Parameter(Mandatory=$true)][string]$ReloadReport,[switch]$WaitForExit)
...
$gaitScript=Join-Path $PSScriptRoot 'ue_m5_vs2_gas_nominal_weight_author.py'
if($gaitData.schema -cne 'HarborCity.M5VS2.GASNominalWeight.Author.v1' -or $gaitData.phase -cne 'GASNominalWeightAuthor' -or $gaitData.status -cne 'PASS' -or
   $gaitCommand.phase -cne 'GASNominalWeightAuthor' -or $gaitCommand.status -cne 'PASS' -or $gaitCommand.exit_code -ne 0 -or
   $gaitData.map -cnotmatch '^/Game/HarborCity/M5VS2/HeroGaitR2/Run_[0-9a-f]{12}/L_HeroGaitR2$' -or $gaitData.assets.Count -ne 8 -or $gaitData.expected_screenshots -ne 9){
    throw 'Require completed unique eight-package R2 gait fixture with nine planned captures.'
}
...
$gaitNormalization=$gaitData.normalization
$gaitAnimationRoot=$gaitNormalization.candidate_space -replace '/BS_Selestia_GASMotion$',''
if($gaitAnimationRoot -cnotmatch '^/Game/HarborCity/M5VS2/HeroSelestia/Animation/GASMotion/Batch_[0-9a-f]{12}$' -or
    $gaitNormalization.candidate_hero -cnotmatch '^/Game/HarborCity/M5VS2/HeroRev2/Review_[0-9a-f]{12}/BP_HeroGait_Normalized$' -or
    $gaitNormalization.candidate_blueprint -cne ($gaitAnimationRoot+'/ABP_Selestia_GASMotion_Normalized')){throw 'Not the exact private normalization candidate.'}
$gaitExpected=@($gaitData.map,($gaitParent+'/BP_HeroGaitR2GameMode'),$gaitNormalization.candidate_hero,
    ($gaitAnimationRoot+'/ABP_Selestia_GASMotion_Normalized'),($gaitAnimationRoot+'/BS_Selestia_GASMotion'),
    ($gaitAnimationRoot+'/Loops/Run_WarpLoop'),($gaitAnimationRoot+'/Loops/Sprint_WarpLoop'),($gaitAnimationRoot+'/Loops/Walk_WarpLoop'))|Sort-Object
$gaitPaths=@($gaitData.assets|ForEach-Object{$_.path}|Sort-Object)
if(@(Compare-Object $gaitExpected $gaitPaths).Count -ne 0){throw 'Unexpected normalized private asset membership.'}
$gaitReloadFile=[IO.Path]::GetFullPath($ReloadReport)
if(-not $gaitReloadFile.StartsWith(([IO.Path]::GetFullPath($gaitDocs)+[IO.Path]::DirectorySeparatorChar),[StringComparison]::OrdinalIgnoreCase) -or
    [IO.Path]::GetFileName($gaitReloadFile) -cne 'author_result.json'){throw 'Require owned fresh-process normalization reload report.'}
$gaitReload=Get-Content -LiteralPath $gaitReloadFile -Raw -Encoding utf8|ConvertFrom-Json
$gaitReloadCommand=Get-Content -LiteralPath (Join-Path (Split-Path $gaitReloadFile -Parent) 'commandlet.json') -Raw -Encoding utf8|ConvertFrom-Json
if($gaitReload.status -cne 'PASS' -or $gaitReload.phase -cne 'GASNominalWeightReload' -or $gaitReload.fresh_process_disk_readback -cne 'PASS' -or
    $gaitReload.author_report -ine $gaitReport -or $gaitReload.author_sha256 -ine (Get-FileHash -LiteralPath $gaitReport).Hash -or
    $gaitReloadCommand.status -cne 'PASS' -or $null -eq $gaitReloadCommand.exit_code -or $gaitReloadCommand.exit_code -ne 0 -or
    $gaitReloadCommand.script_sha256 -ine (Get-FileHash -LiteralPath $gaitScript).Hash){throw 'Exact fresh-process readback missing or stale.'}
if($gaitReload.source_preservation.changed.Count -ne 0){throw 'Reload changed protected sources.'}
...
$gaitOut=Join-Path $gaitDocs ('editor_runtime/'+$gaitRun+'_herogait_normalized_game')
...
$gaitRecord=[ordered]@{milestone='M5_VS2';phase='HeroGaitNormalized';kind='EDITOR_BINARY_GAME_RUNTIME_NOT_PACKAGED';status='STARTING';
    started_at=(Get-Date).ToString('o');executable=$gaitExe;arguments=$gaitArgs;map=$gaitData.map;author_report=$gaitReport;
    author_sha256=(Get-FileHash -LiteralPath $gaitReport).Hash;reload_report=$gaitReloadFile;reload_sha256=(Get-FileHash -LiteralPath $gaitReloadFile).Hash;module_sha256=(Get-FileHash -LiteralPath $gaitModule).Hash;
...
Write-Output "Normalized HeroGait A/B evidence: $gaitOut"
if($WaitForExit){

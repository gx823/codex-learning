"""Independent J/T/U/V/W/X getup retarget actions; no gameplay binding or old asset changes.
...
assert PHASE in tuple('NPCGetUpRetargetR2'+k for k in ('J','T','U','V','W','X'))
...
try:
    ...
    R['template']=dict(path=str(WORK/'tools/ue_m5_vs2_gas_recovery_retarget_author.py'),sha256='d6914b02db146d05da1515a8344109e5722a7844999d22c672650c0eb2ee00d6')
    check('read-only original retarget template unchanged',sha(R['template']['path'])==R['template']['sha256'])
    ...
    if IS_SPRINT:
        ...
        npc_file,npc=latest('NPCIntegrationR2'+LETTER)
        ...
        for path,row in {package(a['path']):a for a in npc['assets']}.items(): protect(path,row['sha256'])

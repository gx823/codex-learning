...
$gaitScript=Join-Path $PSScriptRoot 'ue_m5_vs2_gas_clock_author.py'
if($gaitData.schema -cne 'HarborCity.M5VS2.GASUnifiedClock.Author.v1' -or $gaitData.phase -cne 'GASUnifiedClockAuthor' -or $gaitData.status -cne 'PASS' -or
   $gaitCommand.phase -cne 'GASUnifiedClockAuthor' -or $gaitCommand.status -cne 'PASS' -or $gaitCommand.exit_code -ne 0 -or
   $gaitData.map -cnotmatch '^/Game/HarborCity/M5VS2/HeroGaitR2/Run_[0-9a-f]{12}/L_HeroGaitR2$' -or $gaitData.assets.Count -ne 5 -or $gaitData.expected_screenshots -ne 9){
    throw 'Require completed unique five-package R2 gait fixture with nine planned captures.'
}
...
$gaitClock=$gaitData.clock
$gaitAnimationRoot=$gaitClock.candidate_space -replace '/BS_Selestia_GASMotion$',''
if($gaitAnimationRoot -cnotmatch '^/Game/HarborCity/M5VS2/HeroSelestia/Animation/GASMotion/Batch_[0-9a-f]{12}$' -or
    $gaitClock.candidate_hero -cnotmatch '^/Game/HarborCity/M5VS2/HeroRev2/Review_[0-9a-f]{12}/BP_HeroGait_UniformClock$' -or
    $gaitClock.candidate_blueprint -cne ($gaitAnimationRoot+'/ABP_Selestia_GASMotion_UniformClock')){throw 'Not the exact private unified-clock candidate.'}
$gaitExpected=@($gaitData.map,($gaitParent+'/BP_HeroGaitR2GameMode'),$gaitClock.candidate_hero,
    ($gaitAnimationRoot+'/ABP_Selestia_GASMotion_UniformClock'),($gaitAnimationRoot+'/BS_Selestia_GASMotion'))|Sort-Object
$gaitPaths=@($gaitData.assets|ForEach-Object{$_.path}|Sort-Object)
if(@(Compare-Object $gaitExpected $gaitPaths).Count -ne 0){throw 'Unexpected uniform-clock private asset membership.'}
...
if(-not $gaitReloadFile.StartsWith(([IO.Path]::GetFullPath($gaitDocs)+[IO.Path]::DirectorySeparatorChar),[StringComparison]::OrdinalIgnoreCase) -or
    [IO.Path]::GetFileName($gaitReloadFile) -cne 'author_result.json'){throw 'Require owned fresh-process unified-clock reload report.'}
...
if($gaitReload.status -cne 'PASS' -or $gaitReload.phase -cne 'GASUnifiedClockReload' -or $gaitReload.fresh_process_disk_readback -cne 'PASS' -or
...
$gaitOut=Join-Path $gaitDocs ('editor_runtime/'+$gaitRun+'_herogait_clock_game')
...
$gaitArgs=@(('"'+$gaitProject+'"'),$gaitData.map,'-game','-nosplash','-windowed','-ResX=1920','-ResY=1080','-ForceRes','-language=en',
    '-M5VS2HeroGaitR2','-M5VS2HeroGaitUnifiedClock','-M5VS2AutoQuit',('-M5VS2EvidenceDir="'+$gaitOut+'"'),('-HCM1SaveSlot=HarborCity_VS2_HeroGaitR2_'+$gaitRun),
    ('-abslog="'+(Join-Path $gaitOut 'game.log')+'"'),('-ExecCmds="'+($gaitCommands -join ',')+'"'),"-LocalDataCachePath=$gaitCache/DDC","-ZenDataPath=$gaitCache/Zen")
$gaitRecord=[ordered]@{milestone='M5_VS2';phase='HeroGaitUnifiedClock';kind='EDITOR_BINARY_GAME_RUNTIME_NOT_PACKAGED';status='STARTING';
...
Write-Output "Uniform-clock HeroGait A/B evidence: $gaitOut"
if($WaitForExit){

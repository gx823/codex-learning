...
. (Join-Path $PSScriptRoot 'm5_vs3_release_guards.ps1')
$workspaceRoot = Split-Path $PSScriptRoot -Parent
$projectFile = Join-Path $workspaceRoot 'HarborCity\HarborCity.uproject'
$engineRoot = 'E:\UE_5.8'
$buildRoot = 'E:\GameDev\Builds\HarborCity\M5_VS3'
$cacheRoot = 'D:\GameDev\Cache\Unreal\HarborCity'
$evidenceRoot = Join-Path $workspaceRoot 'docs\HarborCity_M5_VS3\builds'
$buildScript = Join-Path $PSScriptRoot 'build_m5_vs3.ps1'
...
$map = '/Game/HarborCity/M5VS3/Town_169edc3f2e92/L_HarborTown'
...
$workDirectory = Join-Path $workspaceRoot ('docs/HarborCity_M5_VS3/package_work/' + $runId)
if (-not $NoticeFiles.Count) { $NoticeFiles = @((Join-Path $workspaceRoot 'docs/HarborCity_M5_VS3/ASSET_LICENSE_NOTICES.md')) }
...
$buildRoot = [IO.Path]::GetFullPath('E:\GameDev\Builds\HarborCity\M5_VS3').TrimEnd('\')
...
$allowedCookRoot = [IO.Path]::GetFullPath((Join-Path $workspaceRoot 'docs/HarborCity_M5_VS3/package_work')).TrimEnd('\','/') + '\'
...
$expectedPackage = '/Game/HarborCity/M5VS3/Town_169edc3f2e92/L_HarborTown'
$expectedWorld = $expectedPackage + '.L_HarborTown'
$runId = (Get-Date -Format 'yyyyMMdd_HHmmss_fff') + '_' + [Guid]::NewGuid().ToString('N').Substring(0,8)
$runDirectory = Join-Path $workspaceRoot ('docs\HarborCity_M5_VS3\commandlets\' + $runId + '_cooked_maps')
...
New-Item -ItemType Directory -Path $runDirectory | Out-Null
$record = [ordered]@{
    schema_version=6; milestone='M5_VS3'; started_at=(Get-Date).ToString('o'); status='RUNNING'; project=$projectFile;
    engine=$engineRoot; target='HarborCity Win64 Shipping'; maps=@($map);
...
try {
    foreach ($requiredPath in @($projectFile,$buildScript,$dotnetPath,$uatPath,$editorPath,$powershellPath,
        (Join-Path $workspaceRoot 'HarborCity/Content/HarborCity/M5VS3/Town_169edc3f2e92/L_HarborTown.umap'))) {
    ...
    $record.private_review_exclusions = @('HarborCity/Config/DefaultCrypto.ini','HarborCity/Config/DefaultEncryption.ini',
        ...
        'docs/HarborCity_M5_VS3/package_work/**','assets/M5_VS1/hero/Selestia/PrivateCryptoVerification/**')
    ...
    $record.release_scripts = @('package_m5_vs3.ps1','build_m5_vs3.ps1','launch_game_m5_vs1.ps1','launch_editor_game_m5_vs1.ps1','m5_vs3_release_guards.ps1','build.ps1','normalize_response_files.py' | ForEach-Object {
    ...
    if($buildTarget -eq 'Game'){$buildArguments += @('-Configuration','Shipping')}
    ...
    $uatArguments = @('-NoCompile','-NoP4','-UTF8Output','BuildCookRun',"-project=$projectFile",
        '-target=HarborCity','-platform=Win64','-clientconfig=Shipping','-skipbuild','-nocompileeditor',
    ...
    $executables = @(Get-ChildItem -LiteralPath $archiveDirectory -Recurse -File -Filter 'HarborCity-Win64-Shipping.exe' |
        Where-Object { $_.FullName -match '[\\/]HarborCity[\\/]Binaries[\\/]Win64[\\/]HarborCity-Win64-Shipping\.exe$' })

...
# Read-only asset probe in the one completed official sample. No project/script
...
$gasScript = Join-Path $PSScriptRoot 'ue_m5_vs2_gas_animation_probe.py'
...
$gasSelection = $null
if ($SelectionPath) {
    $gasSelection = [IO.Path]::GetFullPath($SelectionPath)
    $gasRelative = [IO.Path]::GetRelativePath([IO.Path]::GetFullPath($gasReports), $gasSelection)
    if ([IO.Path]::IsPathRooted($gasRelative) -or $gasRelative -eq '..' -or $gasRelative.StartsWith('..' + [IO.Path]::DirectorySeparatorChar) -or
        [IO.Path]::GetExtension($gasSelection) -ine '.json' -or -not (Test-Path -LiteralPath $gasSelection -PathType Leaf)) {
        throw 'Selection must be an existing JSON inside current M5-VS2 reports.'
    }
    $gasChosen = (Get-Content -LiteralPath $gasSelection -Raw | ConvertFrom-Json).sequence_paths
    if ($gasChosen -isnot [array] -or $gasChosen.Count -lt 1 -or $gasChosen.Count -gt 32 -or
        @($gasChosen | Select-Object -Unique).Count -ne $gasChosen.Count -or
        @($gasChosen | Where-Object { $_ -isnot [string] -or $_ -notmatch '^/Game/[A-Za-z0-9_/]+$' }).Count -gt 0) {
        throw 'Selection requires 1-32 unique exact /Game package paths; native registry will verify their classes.'
    }
}
...
$gasEvidence = Join-Path $gasReports ('editor_runtime/probe_' + $gasRun + '_GASReadOnly')
...
if ($gasSelection) { $gasArgs += ('-M5GASSelection="' + $gasSelection + '"') }
$gasRecord = [ordered]@{ milestone='M5_VS2'; phase='GAS_READ_ONLY_PROBE'; project=$gasProject; script=$gasScript;
    script_sha256=(Get-FileHash -LiteralPath $gasScript -Algorithm SHA256).Hash; executable=$gasEditor; arguments=$gasArgs;
    plugins_enabled_for_process_only=$gasPluginAudit; selection=$gasSelection; started_at=(Get-Date).ToString('o');
...
try {
    ...
    Write-Output "M5-VS2 GAS read-only probe evidence: $gasEvidence"
    $gasProc.WaitForExit()
    $gasProc.Refresh()

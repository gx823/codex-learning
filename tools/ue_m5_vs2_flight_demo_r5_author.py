...
FlightDemoR5 creates one new map; FlightDemoR5Reload is a fresh read-only process.
"""
...
OWNER='HarborCity_M5VS2_FlightDemoR5'; SCHEMA='HarborCity.M5VS2.FlightDemoR5.Author.v1'
...
assert PHASE in ('FlightDemoR5','FlightDemoR5Reload') and OUT.is_relative_to(DOC/'editor_runtime')
assert not (OUT/'author_result.json').exists(); OUT.mkdir(parents=True,exist_ok=True)
TOKEN=hashlib.sha256(str(OUT).encode()).hexdigest()[:12]; ROOT='/Game/HarborCity/M5VS2/FlightDemoR5/Run_'+TOKEN
R=dict(schema=SCHEMA,art_revision=5,owner=OWNER,phase=PHASE,status='RUNNING',checks=[],assets=[],inputs={},runtime='NOT_RUN',os_input='NOT_RUN',
       ...
       started_utc=dt.datetime.now(dt.timezone.utc).isoformat(),scope='Private copy of current native PASS Live R5 Afternoon; ordinary production input path driven by explicit test director. Not packaged, not OS mouse evidence, no combat/dialogue validation.')
PROTECTED={}; SOURCES={}
...
def resolve_live():
    f,data=latest('author_*_CornerPlayableR5Reload',lambda d:d.get('schema')==LIVE_SCHEMA and d.get('fresh_process_disk_readback')=='PASS')
    ...
    check('source is exact native Live R5 manifest',manifest.get('schema')==LIVE_SCHEMA and manifest.get('status')=='READY_FOR_PLAY_NOT_RUNTIME_TESTED' and manifest.get('expected_npcs')==8 and manifest.get('expected_vehicles')==1)
    check('source manifest belongs to own actual Reload',Path(manifest['reload_report']).resolve()==f.resolve())
    check('R5 source and safety-layer provenance mandatory',manifest.get('art_revision')==5 and manifest.get('source_corner_proof',{}).get('art_revision')==5
          and manifest.get('hero_safety',{}).get('status')=='NATIVE_RELOAD_SELECTED_NOT_ALL_ANGLE_ART_APPROVED'
          and manifest['hero_safety']['blueprint']==manifest['selected_hero'])
    R['hero_safety']=manifest['hero_safety'];R['source_corner_proof']=manifest['source_corner_proof']
    for row in manifest['source_bindings']:protect(row['path'],row['sha256'],row['kind'],**({'package':row['package']} if 'package' in row else {}))
    assets(data['assets']);source_map=manifest['maps']['Afternoon'];check('source exact Afternoon map',source_map==manifest['destination']+'/L_CornerPlayableR5_Afternoon')
...
try:
    ...
    if PHASE=='FlightDemoR5':
        ...
        target=ROOT+'/L_FlightDemoR5';check('unique private namespace absent',not A.does_directory_exist(ROOT) and not disk(target).exists())
        ...
        d.set_actor_label('HC_VS2_FlightDemoR5_InputOnly');d.set_editor_property('tags',[U.Name(OWNER)])
        ...
        f,prior=latest('author_*_FlightDemoR5',lambda d:d.get('schema')==SCHEMA and d.get('owner')==OWNER)
        check('single bounded authored destination',re.fullmatch(r'/Game/HarborCity/M5VS2/FlightDemoR5/Run_[0-9a-f]{12}',prior['destination']) is not None and prior['map']==prior['destination']+'/L_FlightDemoR5' and len(prior['assets'])==1)
        for row in prior['source_bindings']:protect(row['path'],row['sha256'],row['kind'],**({'package':row['package']} if 'package' in row else {}))
        check('R5 and safety-layer proof retained',prior.get('art_revision')==5 and prior.get('source_corner_proof',{}).get('art_revision')==5 and prior.get('hero_safety',{}).get('blueprint')==prior['selected_hero'])
        R['hero_safety']=prior['hero_safety'];R['source_corner_proof']=prior['source_corner_proof']
        ...
        manifest=dict(schema=SCHEMA,art_revision=5,source_corner_proof=R['source_corner_proof'],hero_safety=R['hero_safety'],status='READY_FOR_ENGINE_INPUT_DEMO_NOT_RUNTIME_TESTED',owner=OWNER,kind=R['kind'],input_scope=R['input_scope'],destination=R['destination'],map=R['map'],selected_hero=R['selected_hero'],
                      source_bindings=list(SOURCES.values()),reload_report=str(OUT/'author_result.json'),expected_npcs=8,expected_vehicles=1,save_slot_prefix='HarborCity_VS2_FlightDemo_R5_',
        ...
        target=OUT/'flight_demo_r5_manifest.json';target.write_text(json.dumps(manifest,ensure_ascii=False,indent=2)+'\n','utf-8');R['launch_manifest']=binding(target,'evidence')
except Exception:

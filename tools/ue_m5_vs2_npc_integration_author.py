"""Native Q/R specimen author. Root runs NPCIntegrationQ/R or NPCInPlaceQ/R separately.
...
R = dict(status='RUNNING', checks=[], assets=[], started_utc=dt.datetime.now(dt.timezone.utc).isoformat(),
         ...
         scope='Two isolated native NPC specimens; no map placement or old NPC replacement.')
...
def integrate():
    ...
    cdo.set_editor_property('display_name', '伊蕾娜' if LETTER=='Q' else '莱昂')
    cdo.set_editor_property('role_id', U.Name('HarborHealer' if LETTER=='Q' else 'GuildCourier'))
    cdo.set_editor_property('talkable', True); cdo.set_editor_property('stationary', True)
    cdo.set_editor_property('dialogue_lines', ['海风带来的消息，比船来得更早。' if LETTER=='Q' else '公会的委托刚到，要去街角看看吗？'])
    ...
    choice = 'FLEE' if LETTER=='Q' else 'DEFEND_AGAINST_MELEE'
    ...
    R['specimen'] = dict(blueprint=bp.get_path_name(), generated_class=B.generated_class(bp).get_path_name(),
        ...
        capsule_half_height_cm=half, mesh_z_cm=-half-floor_z, fictional_age=30 if LETTER=='Q' else 34,
...
try:
    ...
    check('run a single specimen per native process', len(phase)==1 and phase[0] in ('NPCIntegrationQ','NPCIntegrationR','NPCInPlaceQ','NPCInPlaceR'),phase)
    LETTER = phase[0][-1]
    if phase[0].startswith('NPCInPlace'): repair_existing()
    else: integrate()
except Exception:

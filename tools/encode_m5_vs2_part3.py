...
if out.name=='M5_VS2_TOWN_PLAYTHROUGH.mp4':
 validation=json.loads((D/'standalone_validation.json').read_text('utf-8-sig'))
 assert validation['status']=='COMPLETE'
 demo=next(x for x in validation['runs'] if x['mode']=='demo')
...
if out.name=='M5_VS2_TOWN_PLAYTHROUGH.mp4':assert 240<=t['duration_seconds']<=360
...
result['capture_metadata_note']='Recorder retains historical milestone M5_VS1 tag; actual VS2 package/map identity is established by standalone validation launch/results, not by that tag.'
if out.name=='M5_VS2_TOWN_PLAYTHROUGH.mp4':result['recording_executable']=demo['launch']['executable']

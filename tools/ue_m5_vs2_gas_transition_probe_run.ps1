...
# Read-only bounded transition original-frame probe in the one completed official sample. No project/script
...
$gasScript = Join-Path $PSScriptRoot 'ue_m5_vs2_gas_transition_probe.py'
...
$gasEvidence = Join-Path $gasReports ('editor_runtime/probe_' + $gasRun + '_GASTransitionReadOnly')
...
$gasRecord = [ordered]@{ milestone='M5_VS2'; phase='GAS_TRANSITION_READ_ONLY_PROBE'; project=$gasProject; script=$gasScript;
    script_sha256=(Get-FileHash -LiteralPath $gasScript -Algorithm SHA256).Hash; executable=$gasEditor; arguments=$gasArgs;
    plugins_enabled_for_process_only=$gasPluginAudit; selection='31 exact candidates: eight starts/stops, six turns, fifteen jump segments, two idle breaks; reads only'; started_at=(Get-Date).ToString('o');
...
try {
    ...
    Write-Output "M5-VS2 GAS transition read-only probe evidence: $gasEvidence"
    $gasProc.WaitForExit()
    $gasProc.Refresh()

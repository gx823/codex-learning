"""Root-only nominal-speed weight A/B: one companion curve; no selected-hero mutation.
...
L,P=U.AnimationLibrary,U.AnimPoseExtensions
...
assert PHASE in ('GASNominalWeightAuthor','GASNominalWeightReload')
...
R=dict(schema='HarborCity.M5VS2.GASNominalWeight.Author.v1',phase=PHASE,status='RUNNING',
...
def semantic_curve_hashes(seq,ignore=()):
    # UE FName identity ignores display case (NameTypes.h / PyWrapperName.cpp).
    # DuplicateAsset actually changed VS2NominalRootSpeed's display spelling to
    # lower case in b283b518, before our native author touched the copy.
    # Compare values/times/functions exactly; normalize only proven FName aliases.
    raw=curve_hashes(seq)
    native_names={U.Name(name):name for name in raw}
    check('unique native curve identities '+seq.get_name(),len(native_names)==len(raw))
    excluded={U.Name(name) for name in ignore}
    result={};aliases=[]
    for name,value in raw.items():
        native_name=U.Name(name)
        if native_name in excluded:continue
        canonical=name.lower()
        check('native FName canonical identity',U.Name(canonical)==native_name,name)
        check('no canonical curve collision',canonical not in result,canonical)
        result[canonical]=value
        if canonical!=name:aliases.append(dict(display_name=name,comparison_name=canonical))
    R.setdefault('curve_name_identity_readbacks',[]).append(dict(asset=package(seq),aliases=aliases,ignored=[str(x) for x in ignore]))
    return result


def curve_differences(before,after):
    differences=[]
    for name in sorted(set(before)|set(after)):
        a,b=before.get(name),after.get(name)
        if a==b:continue
        row=dict(curve=name,missing_before=a is None,missing_after=b is None,fields={})
        if a is not None and b is not None:
            for field in sorted(set(a)|set(b)):
                av,bv=a.get(field),b.get(field)
                if av==bv:continue
                item=dict(before_length=len(av) if isinstance(av,list) else None,after_length=len(bv) if isinstance(bv,list) else None)
                if isinstance(av,list) and isinstance(bv,list):
                    changed=[dict(index=i,before=x,after=y,abs_difference=abs(x-y)) for i,(x,y) in enumerate(zip(av,bv)) if x!=y]
                    item.update(first_differences=changed[:12],differing_paired_values=len(changed),max_abs_difference=max((x['abs_difference'] for x in changed),default=0))
                else:item.update(before=av,after=bv)
...
def save(obj):
    p=package(obj);check('new fixture asset only',p.startswith(ROOT+'/') or p.startswith(DEST+'/') or p==HERO_DEST+'/BP_HeroGait_Normalized')
...
try:
    if PHASE=='GASNominalWeightReload':
        candidates=sorted((DOC/'editor_runtime').glob('author_*_GASNominalWeightAuthor/author_result.json'),key=lambda p:p.stat().st_mtime,reverse=True)
        completed=[]
        for file in candidates:
            prior=document(file);command=file.parent/'commandlet.json'
            if prior.get('status')=='PASS' and command.is_file() and document(command).get('status')=='PASS' and document(command).get('exit_code')==0:
                completed.append((file,prior));break
        check('native completed normalization author exists',len(completed)==1)
        prior_file,prior=completed[0];protect(prior_file);protect(prior_file.parent/'commandlet.json')
        check('same author code',document(prior_file.parent/'commandlet.json')['script_sha256'].lower()==sha(WORK/'tools/ue_m5_vs2_gas_nominal_weight_author.py'))
        check('eight saved private candidate assets',len(prior['assets'])==8)
        for row in prior['assets']:protect(disk(row['path'],'.umap' if row['path']==prior['map'] else '.uasset'),row['sha256'])
        for path,digest in prior['source_preservation']['sha256_before'].items():protect(path,digest)
        n=prior['normalization'];check('same compiled runtime module',sha(PROJECT/'Binaries/Win64/UnrealEditor-HarborCity.dll')==prior['module_sha256'])
        read=json.loads(U.HCM5VS2GASNormalizedEditor.configure_normalized_loops(
            load(n['source_blueprint'],U.AnimBlueprint),load(n['source_space'],U.BlendSpace),
            load(n['candidate_blueprint'],U.AnimBlueprint),load(n['candidate_space'],U.BlendSpace),
            [load(p,U.AnimSequence) for p in n['loops']],False))
        check('fresh disk normalized helper readback',read==n['native'],read)
        bp=load(n['candidate_blueprint'],U.AnimBlueprint);mesh=load(n['mesh'],U.SkeletalMesh)
        read_gas=json.loads(U.HCM5VS2GASMotionEditor.inspect_motion_candidate(bp,mesh,load(ORIGINAL_SPACE,U.BlendSpace),load(n['candidate_space'],U.BlendSpace)))
        read_flight=json.loads(U.HCM5VS2FlightPoseEditor.inspect_flight_graph(bp))
        check('fresh compiled GAS and flight graph',read_gas.get('status')=='PASS' and read_flight==prior['native_retained_flight_graph'])
        ...
        check('fresh fixture candidate bindings',len(directors)==1
              and directors[0].get_editor_property('expected_character_class')==generated(n['candidate_hero'])
              and directors[0].get_editor_property('expected_animation_class')==generated(n['candidate_blueprint'])
              and directors[0].get_editor_property('expected_gait_blend_space')==load(n['candidate_space'],U.BlendSpace))
        R.update(map=prior['map'],assets=prior['assets'],normalization=n,binding=prior['binding'],source=prior['source'],
        ...
        for name in ('HCM5VS2HeroExerciseDirector.h','HCM5VS2HeroExerciseDirector.cpp','HCM5VS2LookAnimInstance.h','HCM5VS2LookAnimInstance.cpp','HCM5VS2GASNormalizedEditor.h','HCM5VS2GASNormalizedEditor.cpp'):
        ...
        # One-variable A/B: fresh loops, space, flight ABP and private Hero only.
        check('fresh normalized animation and Hero namespaces',not A.does_directory_exist(DEST) and not A.does_directory_exist(HERO_DEST))
        source_hero=hero['blueprint'];source_bp=hero['expected']['animation'];source_bs=package(space)
        source_loops=[load(samples[i]['animation'],U.AnimSequence) for i in (0,9,27)]
        helper=WORK/'tools/ue_m5_vs2_gas_motion_author.py';protect(helper)
        tree=ast.parse(helper.read_text('utf-8-sig'))
        functions=[n for n in tree.body if isinstance(n,ast.FunctionDef) and n.name in ('sample_pose_hash','curve_hashes')]
        check('exact known pure preservation helpers',len(functions)==2)
        exec(compile(ast.fix_missing_locations(ast.Module(body=functions,type_ignores=[])),str(helper),'exec'),globals())
        def copy_asset(source,leaf):
            path=(HERO_DEST if leaf=='BP_HeroGait_Normalized' else DEST)+'/'+leaf
            check('new normalized package only',not A.does_asset_exist(path) and not disk(path).exists())
            value=A.duplicate_asset(package(source),path);check('native copy '+leaf,value is not None);return value
        loops=[copy_asset(source,'Loops/'+name) for source,name in zip(source_loops,('Run_WarpLoop','Sprint_WarpLoop','Walk_WarpLoop'))]
        source_curve_before=[semantic_curve_hashes(x) for x in source_loops]
        copy_curve_before=[semantic_curve_hashes(x) for x in loops]
        R['curve_preservation_before_native']=[dict(loop=x.get_name(),differences=curve_differences(a,b))
            for x,a,b in zip(loops,source_curve_before,copy_curve_before)]
        dump()
        new_space=copy_asset(space,'BS_Selestia_GASMotion')
        new_bp=copy_asset(load(source_bp,U.AnimBlueprint),'ABP_Selestia_GASMotion_Normalized')
        normalized=json.loads(U.HCM5VS2GASNormalizedEditor.configure_normalized_loops(
            load(source_bp,U.AnimBlueprint),space,new_bp,new_space,loops,True))
        check('native one-curve normalized candidate',normalized.get('status')=='PASS',normalized)
        R['curve_preservation_after_native']=[]
        curve_exact=True
        for index,(original,loop) in enumerate(zip(source_loops,loops)):
            check('all raw local keys unchanged '+loop.get_name(),sample_pose_hash(original,mesh)==sample_pose_hash(loop,mesh))
            before=source_curve_before[index]
            source_after=semantic_curve_hashes(original)
            target_after=semantic_curve_hashes(loop,('VS2NominalRootWeight',))
            entry=dict(loop=loop.get_name(),source_memory_differences=curve_differences(before,source_after),
                source_to_candidate_differences=curve_differences(before,target_after),
                duplicate_to_authored_differences=curve_differences(copy_curve_before[index],target_after))
            R['curve_preservation_after_native'].append(entry);dump()
            curve_exact=curve_exact and before==source_after==target_after
        check('all existing curve functions and source memory unchanged',curve_exact,R['curve_preservation_after_native'])
        candidate_bp=copy_asset(load(source_hero,U.Blueprint),'BP_HeroGait_Normalized')
        candidate_body=U.get_default_object(candidate_bp.generated_class()).get_editor_property('mesh')
        candidate_body.set_editor_property('anim_class',new_bp.generated_class())
        check('private Hero compile',U.BlueprintEditorLibrary.compile_blueprint(candidate_bp))
        original_body=U.get_default_object(generated(source_hero)).get_editor_property('mesh')
        check('source Hero CDO unchanged in memory',original_body.get_editor_property('anim_class')==generated(source_bp)
              and original_body.get_editor_property('skeletal_mesh_asset')==mesh
              and [original_body.get_material(i) for i in range(original_body.get_num_materials())]==materials)
        hero_class=candidate_bp.generated_class();anim_class=new_bp.generated_class();space=new_space
        candidate_cdo=U.get_default_object(hero_class);candidate_body=candidate_cdo.get_editor_property('mesh')
        check('candidate preserves mesh/materials/movement',candidate_body.get_editor_property('skeletal_mesh_asset')==mesh
              and candidate_body.get_editor_property('anim_class')==anim_class
              and [candidate_body.get_material(i) for i in range(candidate_body.get_num_materials())]==materials
              and candidate_cdo.get_editor_property('walk_speed')==400 and candidate_cdo.get_editor_property('sprint_speed')==650
              and transform_values(candidate_body.get_relative_transform())==transform_values(body.get_relative_transform()))
        native=json.loads(U.HCM5VS2GASMotionEditor.inspect_motion_candidate(new_bp,mesh,load(ORIGINAL_SPACE,U.BlendSpace),space))
        check('all retained GAS graph and sample guards',native.get('status')=='PASS',native.get('error'))
        retained_flight=json.loads(U.HCM5VS2FlightPoseEditor.inspect_flight_graph(new_bp))
        check('flight graph unchanged',retained_flight==flight_native,retained_flight)
        R['normalization']=dict(source_blueprint=source_bp,source_space=source_bs,source_hero=source_hero,
            candidate_blueprint=package(new_bp),candidate_space=package(space),candidate_hero=package(candidate_bp),
            loops=[package(x) for x in loops],mesh=package(mesh),native=normalized,
            source_pose_and_curves_preserved=True,original_selection_unchanged=True,
            changed_only='VS2NominalRootWeight constant one on three private loops; associated candidate references.',
            runtime_sliding_improvement='NOT_RUN')
        for obj in loops+[space,new_bp,candidate_bp]:save(obj)
        samples={row['index']:row for row in native['samples']}
        candidate_hero_package=package(candidate_bp);candidate_anim_package=package(new_bp)
        ...
        check('exact eight new packages',len(R['assets'])==8)
        ...
        R['binding']=dict(character=candidate_hero_package,animation=candidate_anim_package,mesh=hero['expected']['mesh'],
...
if R['status']!='PASS':raise RuntimeError('GASNominalWeight author failed; preserve evidence and all old assets')

...
if(-not $playManifest.StartsWith($playDocs,[StringComparison]::OrdinalIgnoreCase) -or [IO.Path]::GetFileName($playManifest) -cne 'corner_playable_r4_manifest.json' -or $playManifest.Contains('"') -or -not(Test-Path -LiteralPath $playManifest -PathType Leaf)){throw 'Requires the actual private R4 native Reload manifest.'}
...
if($playData.schema -cne 'HarborCity.M5VS2.CornerPlayableR4.v1' -or $playData.status -cne 'READY_FOR_PLAY_NOT_RUNTIME_TESTED' -or $playData.owner -cne 'HarborCity_M5VS2_CornerPlayableR4' -or $playData.kind -cne 'EDITOR_BINARY_GAME_RUNTIME_NOT_PACKAGED' -or $playData.expected_npcs -ne 8 -or $playData.expected_vehicles -ne 1){throw 'Unrecognized private playable manifest.'}
if($playData.destination -cnotmatch '^/Game/HarborCity/M5VS2/WorldRev2/PlayableR4_[0-9a-f]{12}$'){throw 'Invalid private namespace.'}
$playMap=$playData.maps.$Period
if($playMap -cne ($playData.destination+'/L_CornerPlayableR4_'+$Period)){throw 'Wrong saved period map.'}
...
if($playReload.status -cne 'PASS' -or $playReload.phase -cne 'CornerPlayableR4Reload' -or $playReload.schema -cne $playData.schema -or $playReload.fresh_process_disk_readback -cne 'PASS' -or @($playReload.preservation.changed_files).Count -ne 0 -or @($playReload.checks | Where-Object status -CNE 'PASS').Count -ne 0 -or $playCommand.status -cne 'PASS' -or $playCommand.exit_code -ne 0){throw 'Fresh native Reload/process has not passed.'}
...
$playSlot='HarborCity_VS2_PlayR4_'+$playToken+'_Test_'+$Period
if($playData.save_slot_prefix -cne ('HarborCity_VS2_PlayR4_'+$playToken+'_')){throw 'Invalid isolated save namespace.'}
if($ValidateOnly){
...
$playOut=Join-Path $playDocs ($playRun+'_corner_playable_r4_game')

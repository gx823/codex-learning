...
[CmdletBinding()]
param([switch]$Execute,[ValidateRange(1,8)][int[]]$CaseOrders=@(1,2,3,4,5,6,7,8))
$ErrorActionPreference='Stop'
$matrixTools=if((Split-Path $PSScriptRoot -Leaf) -ceq 'staging_npc_remaining'){Split-Path $PSScriptRoot -Parent}else{$PSScriptRoot}
$matrixWork=[IO.Path]::GetFullPath((Split-Path $matrixTools -Parent))
if($matrixWork -ine [IO.Path]::GetFullPath('D:/科研学习/codex学习')){throw 'Wrong HarborCity workspace.'}
if($Execute -and [IO.Path]::GetFullPath($PSScriptRoot) -ine [IO.Path]::GetFullPath($matrixTools)){throw 'Execute requires the reviewed new subset script copied directly into tools; staging is PLAN_ONLY.'}
$matrixRequestedOrders=@($CaseOrders)
if($matrixRequestedOrders.Count -eq 0 -or @($matrixRequestedOrders|Sort-Object -Unique).Count -ne $matrixRequestedOrders.Count){throw 'CaseOrders must contain one or more unique integers from 1 through 8.'}
$matrixSelectedOrders=@($matrixRequestedOrders|Sort-Object)
$matrixIsSubset=$matrixSelectedOrders.Count -ne 8
...
$matrixSelectedPlan=@($matrixPlan|Where-Object {$_.order -in $matrixSelectedOrders})
$matrixCaseResults=@($matrixPlan|ForEach-Object {
    [ordered]@{order=$_.order;phase=$_.phase;site=$_.site;pair=$_.pair;direction=$_.direction;
        selected=($_.order -in $matrixSelectedOrders);status=$(if($_.order -in $matrixSelectedOrders){'NOT_RUN'}else{'NOT_RUN_IN_THIS_RUN'});
        author_report=$null;runtime_report=$null}
})
if(-not $Execute){
    [ordered]@{status='PLAN_ONLY_NOT_RUN';plan=$matrixPlan;requested_case_orders=$matrixRequestedOrders;selected_case_orders=$matrixSelectedOrders;
        selected_run_count=$matrixSelectedPlan.Count;full_plan_count=8;is_subset=$matrixIsSubset;case_results=$matrixCaseResults;run_switch='-Execute';
        scope='Explicit subset of the unchanged eight-case plan; only new evidence from selected cases counts. Requested impulse direction does not guarantee get-up posture.';
...
$matrixOut=Join-Path $matrixRuntime ($matrixRun+'_npc_reaction_subset_r2')
...
$matrixRecord=[ordered]@{schema='HarborCity.M5VS2.NPCReactionMatrixR2.Subset.v1';status='STARTING';started_at=(Get-Date).ToString('o');
    plan=$matrixPlan;full_plan_count=8;requested_case_orders=$matrixRequestedOrders;selected_case_orders=$matrixSelectedOrders;
    selected_run_count=$matrixSelectedPlan.Count;is_subset=$matrixIsSubset;case_results=$matrixCaseResults;historical_results_imported=$false;
    runs=@();observations=@();source_sha256=[ordered]@{};idle_inputs=[ordered]@{};halt=$null;
    ...
    limitations=@('Impulse directions are not semantic get-up poses.','Physics shape filters are not contact-manifold or penetration measurements.',
        'Selected runs do not cover every person x site x get-up pose.','Subset completion never means completion of the full eight-case matrix.',
        'Prior QR/JT or other historical results are not imported into this run.','Explicit fixture RestoreState is not save/load or automatic respawn.');
...
try{
    Assert-NoEngine
    $authorScript=Join-Path $matrixTools 'ue_m5_vs2_npc_reaction_r2_author.py'
    $authorRunner=Join-Path $matrixTools 'ue_m5_vs2_author_run.ps1'
    $launcher=Join-Path $matrixTools 'launch_m5_vs2_npc_reaction_r2.ps1'
    $expected=@{
        (Join-Path $matrixTools 'run_m5_vs2_npc_reaction_matrix_r2.ps1')='298de8506e1481ffbc1adeac0b5920bff2ea793e0ad3d90072f23e74f11ac23e';
    ...
    $sources=@($PSCommandPath,(Join-Path $matrixTools 'run_m5_vs2_npc_reaction_matrix_r2.ps1'),$authorScript,$authorRunner,$launcher,
        (Join-Path $matrixTools 'ue_m5_vs2_npc_cast_review_author.py'),
        (Join-Path $matrixTools 'ue_m5_vs2_npc_idle_r2_author.py'),
    ...
    foreach($plan in $matrixSelectedPlan){
        $caseResult=$matrixRecord.case_results[$plan.order-1];$caseResult.status='RUNNING'
        ...
        $matrixCurrent.author_report=$authorPath;$caseResult.author_report=$authorPath;Save-Matrix
        ...
        $matrixCurrent.runtime_report=$runtimePath;$matrixCurrent.status='PASS';$caseResult.runtime_report=$runtimePath;
        $caseResult.status='PASS';$matrixRecord.completed_run_count++;Save-Matrix
    }
    ...
    $matrixRecord.coverage=[ordered]@{completed_runs=$matrixRecord.completed_run_count;selected_runs=$matrixSelectedPlan.Count;full_plan_runs=8;
        full_matrix_status=$(if($matrixIsSubset){'NOT_RUN_FULL_MATRIX_IN_THIS_RUN'}else{'PENDING_FINAL_COVERAGE_CHECK'});
        people_observed_in_this_run=$people;people_count=$people.Count;all_eight_people_acceptance='NOT_ASSERTED';
    ...
    Require-Matrix ($matrixRecord.completed_run_count -eq $matrixSelectedPlan.Count -and
        @($matrixRecord.case_results|Where-Object {$_.selected -and $_.status -cne 'PASS'}).Count -eq 0) 'Incomplete selected run coverage.'
    if($matrixIsSubset){
        $matrixRecord.status=if(@($missing).Count -eq 0){'PASS_NATIVE_SELECTED_CASES'}else{'PARTIAL_SELECTED_POSE_COVERAGE'}
    }else{
        ...
        $matrixRecord.coverage.full_matrix_status=$matrixRecord.status
    }
}catch{
    if($null -ne $matrixCurrent -and $matrixCurrent.status -ceq 'RUNNING'){
        $matrixCurrent.status='FAIL_OR_STOPPED';$matrixRecord.case_results[$matrixCurrent.order-1].status='FAIL_OR_STOPPED'
    }
    ...
    foreach($case in $matrixRecord.case_results){
        if($case.selected -and $case.status -ceq 'NOT_RUN'){$case.status='NOT_RUN_AFTER_HALT'}
    }
...
if($matrixRecord.status -cin @('PASS_NATIVE_MATRIX','PASS_NATIVE_SELECTED_CASES')){exit 0}
if($matrixRecord.status -cin @('PARTIAL_COVERAGE','PARTIAL_SELECTED_POSE_COVERAGE')){exit 2}
exit 1

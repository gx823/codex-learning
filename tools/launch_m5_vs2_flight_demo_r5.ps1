...
if(-not $demoManifest.StartsWith($demoDocs,[StringComparison]::OrdinalIgnoreCase) -or [IO.Path]::GetFileName($demoManifest) -cne 'flight_demo_r5_manifest.json' -or $demoManifest.Contains('"') -or -not(Test-Path -LiteralPath $demoManifest -PathType Leaf)){throw 'Requires actual private FlightDemoR5 native Reload manifest; deploy this staged launcher into tools first.'}
$demoData=Get-Content -LiteralPath $demoManifest -Raw|ConvertFrom-Json
if($demoData.schema -cne 'HarborCity.M5VS2.FlightDemoR5.Author.v1' -or $demoData.status -cne 'READY_FOR_ENGINE_INPUT_DEMO_NOT_RUNTIME_TESTED' -or $demoData.owner -cne 'HarborCity_M5VS2_FlightDemoR5' -or $demoData.kind -cne 'EDITOR_BINARY_GAME_RUNTIME_NOT_PACKAGED' -or $demoData.input_scope -cne 'ENGINE_INPUT_NOT_OS' -or $demoData.expected_npcs -ne 8 -or $demoData.expected_vehicles -ne 1){throw 'Unrecognized bounded flight-demo manifest.'}
if($demoData.destination -cnotmatch '^/Game/HarborCity/M5VS2/FlightDemoR5/Run_[0-9a-f]{12}$' -or $demoData.map -cne ($demoData.destination+'/L_FlightDemoR5')){throw 'Invalid private map namespace.'}
if($demoData.record_seconds -ne 60 -or $demoData.record_delay -ne 12 -or $demoData.sequence_wall_deadline_seconds -ne 55 -or $demoData.save_slot_prefix -cne 'HarborCity_VS2_FlightDemo_R5_'){throw 'Unexpected capture or private-save contract.'}
...
if($demoReload.status -cne 'PASS' -or $demoReload.phase -cne 'FlightDemoR5Reload' -or $demoReload.schema -cne $demoData.schema -or $demoReload.fresh_process_disk_readback -cne 'PASS' -or @($demoReload.preservation.changed_files).Count -ne 0 -or @($demoReload.checks|Where-Object status -CNE 'PASS').Count -ne 0 -or $demoCommand.status -cne 'PASS' -or $demoCommand.exit_code -ne 0){throw 'Fresh native Reload/process has not passed.'}
...
if($demoData.art_revision -ne 5 -or $demoData.source_corner_proof.art_revision -ne 5 -or $demoData.hero_safety.status -cne 'NATIVE_RELOAD_SELECTED_NOT_ALL_ANGLE_ART_APPROVED' -or $demoData.hero_safety.blueprint -cne $demoData.selected_hero -or $demoData.selected_hero -cnotmatch '^/Game/HarborCity/M5VS2/HeroModesty/Review_[0-9a-f]{12}/BP_HeroSafetyShorts$'){throw 'R5 source and actual safety-layer Reload proof are mandatory.'}
...
$demoSlot='HarborCity_VS2_FlightDemo_R5_Test_'+$demoRun
$demoOut=Join-Path $demoDocs ($demoRun+'_flight_demo_r5_game')
...
if($WaitForExit){
    ...
    if($demoResults.Count -eq 1){
        ...
        if($demoResult.schema -cne 'HarborCity.M5VS2.FlightDemoR5.v1' -or $demoResult.input_scope -cne 'ENGINE_INPUT_NOT_OS'){throw 'Runtime used wrong demo schema; R5 Director support must be compiled before authoring.'}

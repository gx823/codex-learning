"""Evidence-bound new Q/R getup CDO configuration only.
...
Only the two exact VS2 NPC Blueprints may be saved, after private byte backups.
"""
...
assert PHASE in ('NPCGetUpBindingDryRun','NPCGetUpBindingApply')
...
try:
    ...
    guard=WORK/'tools/ue_m5_vs2_npc_gameplay_author.py'
    nodes=[n for n in ast.parse(guard.read_text('utf-8')).body if isinstance(n,ast.FunctionDef) and n.name=='npc_specimen']
    check('one existing strict NPC source guard',len(nodes)==1)
    exec(compile(ast.fix_missing_locations(ast.Module(body=nodes,type_ignores=[])),str(guard),'exec'),globals())
    R['inputs']['pose_helper']=binding(helper);R['inputs']['source_guard']=binding(guard)
    ...
    for letter in ('Q','R'):
        spec=npc_specimen(letter);bp_path=package(spec['blueprint']);bp=load(bp_path)
        ...
        rt_file,rt=latest_pass('author_*_GASRecoveryRetarget'+letter)
        check('exact isolated retarget identity and four actions',rt.get('phase')=='GASRecoveryRetarget'+letter and len(rt['target_audits'])==4 and rt['preservation']['changed_files']==[])
        ...
        physics_evidence=R['inputs']['NPC_'+letter]['physics_repairs'];check('existing native physics repair proof',len(physics_evidence)==1)
        physics=json.loads(Path(physics_evidence[0]['path']).read_text('utf-8'))
        row=next(x for x in physics['assets'] if x['sample']==letter)
        check('simulated chest exists in exact protected physics asset',common['get_up_chest_bone'].casefold() in {b['bone'].casefold() for b in row['native_apply']['bodies']})
    ...
    check('two exact isolated new NPC plans',len(R['plans'])==2 and {p['sample'] for p in R['plans']}=={'Q','R'})
    if APPLY:
        dry_file,dry=latest_pass('author_*_NPCGetUpBindingDryRun')
        ...
        check('exact two native BP saves',len(R['transitions'])==2);R['applied']=True
    ...
    if APPLY and R['status']=='PASS':
        manifest=dict(schema='HarborCity.M5VS2.NPCGetUpBinding.v1',status='PASS',phase=PHASE,evidence=binding(OUT/'author_result.json'),

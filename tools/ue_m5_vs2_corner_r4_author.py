"""Isolated R4 environment author, unchanged 50 m P0 revision-two boundary.
Root alone runs this in native UE; R3 script/layout and every old map are protected.
...
LAYOUT_FILE = DOC / 'research/HARBOR_CORNER_P0_REV2_LAYOUT_R4.json'
...
def resolve_inputs():
    ...
    R['inputs']['r4_author'] = binding(Path(__file__).resolve())
...
def island_elevation(x,y,r,height,seed):
    # Same island footprint, unequal crests and secondary shoulders. This is
    # actual 3D topography; the high-frequency detail stays below the silhouettes.
    peaks=max(.70*math.exp(-((x+.47)**2/.055+(y+.18)**2/.17)),
              .96*math.exp(-((x+.02)**2/.055+(y-.08)**2/.095)),
              .58*math.exp(-((x-.49)**2/.080+(y+.12)**2/.18)),
              .42*math.exp(-((x-.27)**2/.080+(y-.43)**2/.052)))
    ridge=.10*abs(math.sin(x*12+y*6+seed))+.028*math.sin(x*29-seed)*math.sin(y*23+seed)
    return height*max(0.,peaks+ridge+.04)*max(0.,1-r**3)
...
def terrain_detail_material(name):
    ...
    check('R4 terrain sampled base output',E.connect_material_property(detail,'',U.MaterialProperty.MP_BASE_COLOR))
    check('R4 terrain roughness output',E.connect_material_property(rough,'',U.MaterialProperty.MP_ROUGHNESS))
    E.recompile_material(material);save_owned(material)
    R.setdefault('r4_terrain_materials',[]).append(dict(path=material.get_path_name(),
...
def author_coast_and_horizon(layout):
    material=terrain_detail_material('M_R4_CoastalStrata')
...
// Full sphere, original analytic cumulus. R3 used only a height ramp for the
// entire union, erasing each lobe's internal form. R4 retains distinct rounded
// volumes, small irregular contour detail, and a shared flatter shadow base.
for(int g=0;g<9;g++) {
 float seed=frac(sin((g+1)*13.719)*17341.317);
 float cx=frac(0.053+g*.113+seed*.017); float cy=.095+.16*frac(g*.618+seed*.1);
 float w=.015+.010*frac(g*.347+seed); float ht=.055+.037*frac(g*.723+seed);
 float dx=abs(u-cx); dx=min(dx,1.0-dx);
 float base=1.0-smoothstep(.96,1.015,length(float2(dx/(w*1.9),(h-cy)/(ht*.30))));
 float blob=base;
 float formSum=base*.11; float formWeight=base;
 for(int k=0;k<8;k++) {
  float n=frac(sin((g*19+k+1)*17.23)*31415.17);
  float px=cx+(k-3.5)*w*.46+w*(n-.5)*.19;
  float py=cy+ht*(.14+.48*sin((k+.4)*.42)+n*.22);
  float sx=w*(.47+.20*n); float sy=ht*(.40+.25*frac(n*3.73));
  float bx=u-px; bx-=floor(bx+.5);
  float2 v=float2(bx/sx,(h-py)/sy);
  float ripple=.025*sin(bx/sx*12.+g*2.1)*sin((h-py)/sy*9.+k*1.7);
  float q=length(v)+ripple;
  float lobe=1.0-smoothstep(.965,1.015,q);
  blob=max(blob,lobe);
  float depth=sqrt(saturate(1.-dot(v,v)));
  float3 normal=normalize(float3(v,depth+.0001));
  float illumination=.25+.63*saturate(dot(normal,normalize(float3(-.50,.65,.57))));
  float weight=lobe*(.16+pow(saturate(depth),4.)*2.4);
  formSum+=weight*illumination; formWeight+=weight;
 }
 blob*=smoothstep(cy-ht*.27,cy-ht*.19,h)*saturate(CloudCoverage);
 float upper=smoothstep(cy-ht*.07,cy+ht*.22,h);
 float groupShade=lerp(.08,formSum/max(formWeight,.00001),upper);
 // Shade blends continuously between groups. No winning-group branch seam.
 clouds=max(clouds,blob);
 shadeSum+=blob*groupShade;shadeWeight+=blob;
}
float shade=shadeSum/max(shadeWeight,0.000001);
float3 cloudColor=lerp(CloudShade,CloudLight,smoothstep(.10,.69,shade));
...
'''
    ...
    R['r4_cloud_shader']=dict(material=mat.get_path_name(),hlsl_sha256=hashlib.sha256(code.encode('utf-8')).hexdigest(),
        groups=9,lobes_per_group=8,depth_model='Analytic lobe sphere normals; not volumetric cloud scattering',
        changes='Asymmetric silhouette, per-lobe shading, base shadow, local contour ripple, sub-cell star jitter',

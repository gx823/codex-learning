"""Isolated 50 m P0 revision-two author. Root alone runs this in native UE.
...
LAYOUT_FILE = DOC / 'research/HARBOR_CORNER_P0_REV2_LAYOUT_R3.json'
...
def resolve_inputs():
    ...
    for file in (PROJECT/'Content/HarborCity/M5VS2/WorldRev2').rglob('*'):
        if file.is_file() and file.suffix in ('.uasset', '.umap'):
            protect_file(file)
    check('six complete instances of four existing source houses', len(layout['houses']) == 6
...
def foundation_geometry(outline):
    ...
    material=simple_material('M_OriginalWeatheredQuay',(.24,.265,.265),vertex_color=True)
...
def island_geometry(size, height, seed):
    # Three unequal offset ridges, not a single radial central cone. Horizon
    # remains non-playable geometry and retains each approved XY footprint.
    segments, rings = 64, 24
    def elevation(x,y,r):
        peaks=max(.80*math.exp(-((x+.43)**2/.065+(y+.10)**2/.15)),
                  math.exp(-((x-.04)**2/.075+(y-.13)**2/.10)),
                  .67*math.exp(-((x-.46)**2/.075+(y+.04)**2/.15)))
        ridge=.055*math.sin(x*19+seed)*math.sin(y*13+seed*.6)
        return height*max(0.,peaks+ridge+.07)*max(0.,1-r**4)
    vertices = [(0,0,elevation(0,0,0))]; colors=[(.18,.28,.16)]; triangles=[]
    for ring in range(1,rings+1):
        ...
        for i in range(segments):
            ...
            tone=.022*math.sin(x*17+seed)*math.sin(y*14-seed)
            base=(.16,.24,.14) if r<.78 and z>height*.1 else (.25,.265,.24)
...
def coastal_rock_geometry(size,height,seed):
    # Steep irregular waterline, no broad nearly-horizontal circular skirt.
    segments=13;vertices=[];colors=[];triangles=[]
    for ring,(radius,z) in enumerate(((.73,-110),(1.,35),(.88,height*.48),(.57,height*.85),(.15,height))):
        for i in range(segments):
            theta=2*math.pi*i/segments
            edge=1.+.09*math.sin(3*theta+seed)+.06*math.sin(5*theta+seed*.4)
            dx=ring*.024*math.cos(seed);dy=ring*.026*math.sin(seed)
            vertices.append((size[0]*.5*(radius*edge*math.cos(theta)+dx),
                             size[1]*.5*(radius*edge*math.sin(theta)+dy),
                             z+12*math.sin(2*theta+seed+ring*.5)))
            tone=.025*math.sin(i*1.73+ring+seed)
            colors.append((.255+tone,.27+tone,.255+tone))
    for ring in range(4):
...
def author_coast_and_horizon(layout):
    material=simple_material('M_OriginalCoastalStone',(.24,.27,.28),vertex_color=True)
...
// Original analytic cumulus groups with broad lobes and flat low bases.
// Complete sphere, no external background photograph or painted city image.
for(int g=0;g<9;g++) {
 float cx=frac(0.071+g*0.113); float cy=0.105+0.10*frac(g*0.618);
 float w=0.022+0.012*frac(g*0.347); float ht=0.07+0.035*frac(g*0.723);
 float dx=abs(u-cx); dx=min(dx,1.0-dx);
 float base=1.0-smoothstep(0.88,1.04,length(float2(dx/(w*1.8),(h-cy)/(ht*.42))));
 float blob=base;
 for(int k=0;k<5;k++) {
  float px=cx+(k-2)*w*.55; float py=cy+ht*(.28+.5*sin((k+1)*.72));
  float bx=abs(u-px);bx=min(bx,1.0-bx);
  float q=length(float2(bx/(w*.72),(h-py)/(ht*(.52+.07*k))));
  blob=max(blob,1.0-smoothstep(.92,1.02,q));
 }
 blob*=smoothstep(cy-ht*.45,cy-ht*.2,h)*saturate(CloudCoverage);
 // A winning-group branch changed shade discontinuously at overlaps, even
 // when both masks were opaque. Blend shade continuously, keep union mask.
 clouds=max(clouds,blob);
 shadeSum+=blob*saturate((h-cy+ht*.2)/(ht*1.5));shadeWeight+=blob;
}
float shade=shadeSum/max(shadeWeight,0.000001);
float3 cloudColor=lerp(CloudShade,CloudLight,smoothstep(.12,.82,shade));
...
float star=(1.0-smoothstep(.08,.21,length(frac(starUV)-.5)))*step(.994,hash)*smoothstep(.05,.35,h);
...
try:
    ...
    if PHASE=='CornerVTwo':
        ...
        check('six complete house instances and two existing NPC bindings',len(R['houses'])==6 and len(R['npc_bindings'])==2)
